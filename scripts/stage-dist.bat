...
if not exist "%BUILD_DIR%\OMoutasEM4Tweaks\Hooks\BorderlessWindowFix.dll" (
    echo Missing hook DLL: "%BUILD_DIR%\OMoutasEM4Tweaks\Hooks\BorderlessWindowFix.dll"
    popd
    exit /b 1
)
...
mkdir "%HOOKS_DIR%" || exit /b 1
mkdir "%LOGS_DIR%" || exit /b 1

copy /y "%BUILD_DIR%\OMoutasEM4Tweaks.exe" "%DIST_DIR%\" >nul || exit /b 1
copy /y "%BUILD_DIR%\OMoutasEM4Tweaks\Hooks\BorderlessWindowFix.dll" "%HOOKS_DIR%\" >nul || exit /b 1
